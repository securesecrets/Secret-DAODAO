...
impl<InstantiateExt, ExecuteExt, QueryExt, MetadataExt>
    Snip721Contract<InstantiateExt, ExecuteExt, QueryExt, MetadataExt>
where
    QueryExt: JsonSchema,
    MetadataExt: Clone + Serialize + for<'a> Deserialize<'a>,
{
    ...
    pub fn mint_clones(
        ...
        let mut mints: Vec<Mint<MetadataExt>> = Vec::new();
        for _ in 0..quantity {
            mints.push(Mint {
                token_id: None,
                owner: owner.clone(),
    ...
    ///
    /// * `deps` - a reference to Extern containing all the contract's external dependencies
    pub fn query_contract_creator(&self, deps: Deps) -> StdResult<Binary> {
    ...
    pub fn query_contract_info(&self, storage: &dyn Storage) -> StdResult<Binary> {
    ...
    ///
    /// * `storage` - a reference to the contract's storage
    pub fn query_config(&self, storage: &dyn Storage) -> StdResult<Binary> {
        let config: Config = load(storage, CONFIG_KEY)?;
    ...
    ///
    /// * `deps` - a reference to Extern containing all the contract's external dependencies
    pub fn query_minters(&self, deps: Deps) -> StdResult<Binary> {
        let minters: Vec<CanonicalAddr> = may_load(deps.storage, MINTERS_KEY)?.unwrap_or_default();
    ...
    /// * `storage` - a reference to the contract's storage
    /// * `token_id` - string slice of the token id
    pub fn query_nft_info(&self, storage: &dyn Storage, token_id: &str) -> StdResult<Binary> {
            ...
            viewer
                ...
                .map_or_else(
                    ...
                    || {
                        ...
                        ViewingKey::check(deps.storage, owner_addr.as_str(), key)
                            .map_err(|_| StdError::generic_err(VIEWING_KEY_ERR_MSG))?;
                        Ok::<(bool, Option<CanonicalAddr>), StdError>((
                            true,
                            Some(owner_raw.clone()),
                        ))
                    },
    ...
    ///
    /// * `storage` - a reference to the contract's storage
    pub fn query_is_unwrapped(&self, storage: &dyn Storage, token_id: &str) -> StdResult<Binary> {
        let config: Config = load(storage, CONFIG_KEY)?;
        let get_token_res = self.get_token(storage, token_id, None);
        match get_token_res {
            Err(err) => match err {
    ...
    pub fn query_code_hash(&self, deps: Deps, contract: &str) -> StdResult<Binary> {
    ...
    fn process_cw721_owner_of(
        ...
        if self
            ...
            .is_ok()
        {
            let (owner, mut approvals, mut operators) = self.get_owner_of_resp(
                deps,
                block,
                &prep_info.token,
                opt_viewer,
            ...
            if perm.address == global_raw {
                ...
                if has_some {
                    approvals.push(Snip721Approval {
                        ...
                        view_owner_expiration: perm.expirations[perm_type_info.view_owner_idx]
                            .take(),
    ...
    fn check_view_supply(
        ...
        if !is_auth {
            let querier = self.get_querier(deps, viewer, from_permit)?;
    ...
    fn check_status(&self, contract_status: u8, priority: u8) -> StdResult<()> {
    ...
    fn set_metadata_impl(
        ...
        self.enforce_metadata_field_exclusion(metadata)?;
    ...
    fn process_accesses(
        ...
        if alt_auth_list.has_update {
            ...
            let mut auth_list: Vec<AuthList> =
                may_load(&auth_store, owner_slice)?.unwrap_or_default();
    ...
    fn transfer_impl(
        ...
        let (mut token, idx) = self.get_token_if_permitted(
            deps.as_ref(),
            block,
            &token_id,
            Some(sender),
        ...
        for addr in update_addrs.into_iter() {
            let inv_upd = if let Some(inv) =
                inv_updates.iter_mut().find(|i| i.inventory.owner == addr)
            {
                inv
            } else {
    ...
    fn send_list(
        ...
        if let Some(xfers) = transfers {
            ...
            for send in snds.into_iter() {
                ...
                for token_id in send.token_ids.into_iter() {
                    let owner_raw = self.transfer_impl(
                        &mut deps,
                        &env.block,
                        config,
                        &sender,
                ...
                messages.extend(self.receiver_callback_msgs(
                    &mut deps,
                    env.clone(),
                    &send.contract,
                    &contract_raw,
        ...
        save(deps.storage, CONFIG_KEY, &config)?;
        self.update_owner_inventory(deps.storage, &inv_updates, num_perm_types)?;
        Ok(messages)
    }
    ...
    fn burn_list(
        ...
        for burn in burns.into_iter() {
            for token_id in burn.token_ids.into_iter() {
                let (token, idx) = self.get_token_if_permitted(
                    deps.as_ref(),
                    block,
                    &token_id,
                    Some(sender),
                    ...
                    .find(|i| i.inventory.owner == token.owner)
                {
                    ...
                    inv_updates.last_mut().ok_or_else(|| {
                        StdError::generic_err(
                            "Just pushed an InventoryUpdate so this can not happen",
                        )
                    })?
                };
                inv_upd.inventory.remove(deps.storage, idx, false)?;
    ...
    fn mint_list(
        ...
        for mint in mints.into_iter() {
            ...
            config.token_cnt = config.token_cnt.checked_add(1).ok_or_else(|| {
                StdError::generic_err(
                    "Attempting to mint more tokens than the implementation limit",
                )
            })?;
            ...
            // save new token info and token_Extension info
            let token_key = config.mint_cnt.to_le_bytes();
            let mut info_store = PrefixedStorage::new(deps.storage, PREFIX_INFOS);
            json_save(&mut info_store, &token_key, &token)?;
            self.token_extension_info
                .insert(deps.storage, &id, &mint.extension)?;
            // add token to owner's list
            let inventory =
                if let Some(inv) = inventories.iter_mut().find(|i| i.owner == token.owner) {
                    inv
                } else {
            ...
            if let Some(pub_meta) = mint.public_metadata {
                self.enforce_metadata_field_exclusion(&pub_meta)?;
                let mut pub_store = PrefixedStorage::new(deps.storage, PREFIX_PUB_META);
                save(&mut pub_store, &token_key, &pub_meta)?;
                self.pub_metadata.insert(deps.storage, &id, &pub_meta)?;
            }
            if let Some(priv_meta) = mint.private_metadata {
                self.enforce_metadata_field_exclusion(&priv_meta)?;
                let mut priv_store = PrefixedStorage::new(deps.storage, PREFIX_PRIV_META);
                save(&mut priv_store, &token_key, &priv_meta)?;
                self.priv_metadata.insert(deps.storage, &id, &priv_meta)?;
            }
    ...
    fn enforce_metadata_field_exclusion(&self, metadata: &Metadata) -> StdResult<()> {
    ...
    pub fn dossier_list(
        ...
        for id in token_ids.into_iter() {
            ...
            let (mut token, idx) = self.get_token(deps.storage, &id, opt_err)?;
            let owner_slice = token.owner.as_slice();
            ...
            let owner_inf = if let Some(inf) = owner_cache.iter().find(|o| o.owner == token.owner) {
                ...
                let mut all_perm: Vec<Permission> =
                    json_may_load(&all_store, owner_slice)?.unwrap_or_default();
                let (inventory_approvals, view_owner_exp, view_meta_exp) = self
            ...
            let private_metadata = if let Err(err) = self.check_perm_core(
                deps,
                block,
                &token,
                &id,
            ...
            let (token_approv, token_owner_exp, token_meta_exp) = self.gen_snip721_approvals(
                deps.api,
                block,
                &mut token.permissions,
                incl_exp,
