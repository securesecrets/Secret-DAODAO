...
pub fn execute_vote(
    ...
    // BALLOTS.(deps.storage, (proposal_id, &info.sender), |bal| match bal {
    //     Some(current_ballot) => {
    ...
    //     }),
    // })?;
    // let mut current_ballot = BALLOTS.get(deps.storage, &(proposal_id, info.sender.clone()));
...
//     //     },
//     // )?;
//     let mut ballot = BALLOTS.get(deps.storage, &(proposal_id, info.sender.clone()));
...
//             // })?;
//             let mut proposals = PROPOSALS.get(deps.storage, &proposal_id);
