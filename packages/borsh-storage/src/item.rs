...
mod test {
    ...
    #[rstest]
    #[case(CONFIG)]
    fn serialization(#[case] config: Item<Config>) {}

    #[apply(serialization)]
    fn save_and_load(#[case] config: Item<Config>) {
    ...
    #[apply(serialization)]
    fn remove_works(#[case] config: Item<Config>) {
        let mut store = MockStorage::new();
    ...
    #[apply(serialization)]
    fn update_success(#[case] config: Item<Config>) {
        let mut store = MockStorage::new();
    ...
    #[apply(serialization)]
    fn update_can_change_variable_from_outer_scope(#[case] config: Item<Config>) {
        let mut store = MockStorage::new();
        let cfg = Config {
            owner: "admin".to_string(),
            max_tokens: 1234,
    ...
    #[apply(serialization)]
    fn update_does_not_change_data_on_error(#[case] config: Item<Config>) {
        let mut store = MockStorage::new();
    ...
    #[apply(serialization)]
    fn update_supports_custom_errors(#[case] config: Item<Config>) {
        #[derive(Debug)]
        enum MyError {
            Std(StdError),
            Foo,
    ...
    #[apply(serialization)]
    fn readme_works(#[case] config: Item<Config>) -> StdResult<()> {
        let mut store = MockStorage::new();

        // may_load returns Option<T: BorshSerialize + BorshDeserialize>, so None if data is missing
